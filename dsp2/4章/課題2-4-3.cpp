...
void idwt(vector<double> out, vector<double>& original, int level) {
    // スケーリング，ウェーブレット関数
    auto scaling = [] (double a, double b) {return (1/sqrt(2))*(a+b);};
    auto wavelet = [] (double a, double b) {return (1/sqrt(2))*(a-b);};
    int max_j = 1;
    for(int i=0; i<level; i++) {
        for(int j=0; j<max_j; j++) {
            original[2*j] = scaling(out[j], out[j+max_j]);
            original[2*j+1] = wavelet(out[j], out[j+max_j]);
        }
        max_j *= 2;
        for(int j=0; j<max_j; j++) {
            out[j] = original[j];
...
int main() {
    ...
    for(int p=0; p<8; p++) printf("%lf ", b[p]);
    printf("\nafter: ");
    for(int p=0; p<8; p++) printf("%lf ", c[p]);
    printf("\n\n");
    return 0;
}