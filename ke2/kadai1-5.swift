...
func main() -> Int {
    ...
    for i in 0 ..< sample.count {
        ...
        for j in 0 ..< sample[0].count {
            let ans = Bayes_identification(Target: sample[i][j], Average: allAve, EigenValue: allEgn, EigenVector: allVct)
            if (i == ans) {
                ans_count += 1
            }
            else {
...
func Bayes_identification(Target sampleData :[Double], Average aveData :[[Double]], EigenValue egnData :[[Double]], EigenVector vctData :[[[Double]]]) -> Int {
    let a = 0.5;
    var index = -1
    var ans = 10000000.0
    var xSubm: [Double]
    ...
    for i in 0 ..< aveData.count {
        ...
        var lambda = 0.0
        xSubm = matrix_sub(Target: sampleData, Target2: aveData[i])
        /* jは179が理想だけど、値が小さいものは邪魔なので100程度に落とす */
        for j in 0 ..< 180 {
            var tmp_xmek = 0.0
            for m in 0 ..< 196 {
                tmp_xmek += xSubm[m] * vctData[i][m][j]
            ...
            if (j == 0) {
                ...
                lambda = lambda * egnData[i][j]
            }
        }
        tmp_d = tmp_d * a;
        tmp_d += (1 - a) * log(lambda + 0.0000000001)
        if (ans > tmp_d) {
            ans = tmp_d
            index = i
        }
